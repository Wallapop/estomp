...
get_messages (_, Messages, []) ->
	Messages;
get_messages (Connection, Messages, Response) ->
			{Frame, Rest}=parse_frame(Response),
			get_messages(Connection, lists:append(Messages, [Frame]), get_rest(Rest)).
...
get_command(Message) ->
	get_command(Message, #frame{}).

get_command ([], Frame) ->
	Frame;
get_command ([H|T], Frame) ->
	case (H) of
		10 -> get_headers(T, Frame);
		_ -> get_command(T, #frame{command=lists:append([Frame#frame.command, [H]])})
	end.

get_message_body([], Frame) ->
  {Frame, []};
get_message_body([0|T], Frame) ->
  {Frame, T};
get_message_body([H|T], Frame) ->
  get_message_body(T, Frame#frame{body=lists:append([Frame#frame.body, [H]])}) .

%% extract headers as a blob of chars, after having iterated over . . .
get_headers(Message, Frame) ->
	get_headers2(Message, [], Frame).

get_headers2 (Message, Headers, Frame) ->
	get_headers2 (Message, Headers, -1, Frame).
get_headers2 ([H|T], Headers, LastChar, Frame) ->
	case ({H, LastChar}) of
		{10, 10} ->
    ParsedHeaders = parse_headers(Headers),
    get_message_body(T, Frame#frame{headers=ParsedHeaders});
		{_, _} -> get_headers2(T, lists:append([Headers, [H]]), H, Frame)
	end.

parse_headers(RawHeaders) ->
  get_headers_from_raw_src([], RawHeaders).
